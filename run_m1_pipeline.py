...
if __name__ == "__main__":
    ...
    try:
        ...
        # 阶段 1: Extract - 读取数据
        logger.info("\n[阶段 1/3] Extract - 读取数据...")
        df_raw = pipeline.extract()

        # 阶段 2: Transform - 构建清洗表达式
        logger.info("\n[阶段 2/3] Transform - 构建清洗表达式...")
        df_cleaned = pipeline.transform(df_raw)

        # 阶段 3: Load - 分区写入并执行复杂清洗
        logger.info("\n[阶段 3/3] Load - 分区写入并执行复杂清洗...")
        pipeline.load(df_raw)  # 传入原始 df，load() 内部会分区后执行清洗

        time_end_etl = time.time()
        logger.info(f"\n✅ ETL 流程完成！耗时：{time_end_etl - time_start_etl:.2f} 秒")

        # 合并所有分区文件
        logger.info("\n[合并] 合并所有分区文件...")
        time_start_merge = time.time()
        partition_pattern = os.path.join(OUTPUT_ROOT, "behavior_type=*", "data.parquet")
        ...
        logger.info(f"✅ 已合并输出为 {FINAL_OUTPUT}（耗时：{time_end_merge - time_start_merge:.2f} 秒）\n")
